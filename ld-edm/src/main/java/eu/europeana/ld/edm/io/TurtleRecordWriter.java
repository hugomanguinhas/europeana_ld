...
public class TurtleRecordWriter implements IterativeRecordWriter, RecordWriter
{
    private BufferedWriter     _ps;
    private Map<String,String> _decl        = new HashMap();
    private boolean            _closeNested = true;

    public TurtleRecordWriter(boolean close) { _closeNested = close; }
    public TurtleRecordWriter()              { this(true);           }
     ...
     **************************************************************************/
    @Override
    public synchronized void write(Resource r, OutputStream out)
           throws IOException
    {
        if ( _ps != null ) { 
            throw new IOException("Writer already in use in iterator mode");
        }

        _ps  = new BufferedWriter(new OutputStreamWriter(out, "UTF-8")
                                , EDMIOUtils.BUFFER_SIZE);
        try     { write(r.getModel());  }
        finally { close();              }
    }
    ...
    public void close() throws IOException
    {
        try {
            ...
            if (_closeNested) { closeQuietly(_ps); }
            _decl.clear();
            _ps  = null;
        }
    }
